...
def typed(func):
	def rv(*args, **kwargs):
		...
		for i in range(len(rv._pos_types)):
			...
			if str(type(args[i])) != rv.pos_types[i]:
		...
		for i in range(len(rv._pos_types), len(rv._pos_types) + rv._pos_kw_overlap_len):
			...
			if key in visited_keys:
				return func(*args, **kwargs)
			if (str(type(args[i])) != rv._kw_types[key]):
		...
		for key in kwargs:
			...
			if key in rv._kw_types:
				if(str(type(kwargs[key])) != rv.kw_types[key]):
					raise _bad_key_type(key, func.__name__)
		func_rv = func(*args, **kwargs)
		if str(type(func_rv) != rv._rv_type):
			raise TypeError(func.__name__, 'evaluated to unexpected return type', str(type(func_rv)))
		return rv
	...
	for i in range(len(parameters)):
		param = parameters[next(param_iter)]
